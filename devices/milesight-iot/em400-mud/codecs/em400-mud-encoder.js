...
/**
 * install height
 * @param {number} install_height unit: mm, range: [30, 4500]
 * @example { "install_height": 20 }
 */
function setInstallHeight(install_height) {
    if (install_height < 30 || install_height > 4500) {
        throw new Error("install_height must be in range [30, 4500]");
    }
...
/**
 * working mode
 * @param {number} working_mode values: (0: standard, 1: bin, 2: parking)
 * @example { "working_mode": 0 }
 */
function setWorkingMode(working_mode) {
    var working_mode_map = { 0: "standard", 1: "bin", 2: "parking" };
 ...
 * @param {number} standard_mode_alarm_config.threshold_min unit: mm, range: [0, 4500]
 * @param {number} standard_mode_alarm_config.threshold_max unit: mm, range: [0, 4500]
...
function setStandardModeAlarmConfig(standard_mode_alarm_config) {
    ...
    if (threshold_min < 0 || threshold_min > 4500) {
        throw new Error("standard_mode_alarm_config.threshold_min must be in range [0, 4500]");
    }
    if (threshold_max < 0 || threshold_max > 4500) {
        throw new Error("standard_mode_alarm_config.threshold_max must be in range [0, 4500]");
    }
