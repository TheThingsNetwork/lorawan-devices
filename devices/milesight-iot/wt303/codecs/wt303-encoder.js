 ...
 * @param {string} plan_config._item.name_first
 * @param {string} plan_config._item.name_last
...
function setPlanTemperatureControlConfig(plan_id, plan_config) {
    ...
    var heating_temperature = plan_config.heating_temperature || 0;
    var cooling_temperature_enable = plan_config.cooling_temperature_enable;
    var cooling_temperature = plan_config.cooling_temperature || 0;
    var temperature_tolerance_enable = plan_config.temperature_tolerance_enable;
    var temperature_tolerance = plan_config.temperature_tolerance || 0;
    ...
    if (fan_mode_values.indexOf(fan_mode) === -1) {
        throw new Error("plan_config.schedule_config._item.fan_mode must be one of " + fan_mode_values.join(", "));
    }
    if (getValue(enable_values, heating_temperature_enable) === 1 && (heating_temperature < 5 || heating_temperature > 35)) {
        throw new Error("plan_config.schedule_config._item.heating_temperature must be between 5 and 35");
    }
    if (getValue(enable_values, cooling_temperature_enable) === 1 && (cooling_temperature < 5 || cooling_temperature > 35)) {
        throw new Error("plan_config.schedule_config._item.cooling_temperature must be between 5 and 35");
    }
    var temperature_tolerance_value = temperature_tolerance * 100;
    if (getValue(enable_values, temperature_tolerance_enable) === 1 && (temperature_tolerance_value < 10 || temperature_tolerance_value > 500)) {
        throw new Error("plan_config.schedule_config._item.temperature_tolerance must be between 0.1 and 5");
    }
