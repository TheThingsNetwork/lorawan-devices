      ...
      switch (bytes[3 + blocklength]) {
          case 0x01:
              ...
              for (var j = 0; j < filterlength; j++) {
                  if (j == 0)
                      filtervalue = (Array(2).join(0) + (bytes[6 + blocklength + j].toString(16).toUpperCase())).slice(-2);
                  else
                      filtervalue = filtervalue + (Array(2).join(0) + (bytes[6 + blocklength + j].toString(16).toUpperCase())).slice(-2);
              }
...
function Multi_Devices_Type_proc(bytes) {
  ...
  for (var i = 0; i < (Multi_Devices_Type_Msg.number); i++) {
      ...
      for (var j = 0; j < le - 2; j++) {
          if (j == 0)
              typedata = (Array(2).join(0) + (bytes[2 + i * le + j].toString(16).toUpperCase())).slice(-2);
          else
              typedata = typedata + (Array(2).join(0) + (bytes[2 + i * le + j].toString(16).toUpperCase())).slice(-2);   
      }
...
function Device_Type_1_proc(bytes) {
  ...
  for (var i = 0; i < (Device_Type_1_Msg.number); i++) {
      ...
      for (var j = 0; j < le - 1; j++) {
          if (j == 0)
              type1data = (Array(2).join(0) + (bytes[1 + (5*i) + j].toString(16).toUpperCase())).slice(-2);
          else
              type1data = type1data + (Array(2).join(0) + (bytes[1 + (5*i) + j].toString(16).toUpperCase())).slice(-2);
      }
...
function Device_Type_2_proc(bytes) {
  ...
  for (var i = 0; i < (Device_Type_2_Msg.number); i++) {
      ...
      for (var j = 0; j < le - 1; j++) {
          if (j == 0)
              type2data = (Array(2).join(0) + (bytes[1 + (5*i) + j].toString(16).toUpperCase())).slice(-2);
          else
              type2data = type2data + (Array(2).join(0) + (bytes[1 + (5*i) + j].toString(16).toUpperCase())).slice(-2);
      }
...
function Device_Type_3_proc(bytes) {
  ...
  for (var i = 0; i < (Device_Type_3_Msg.number); i++) {
      ...
      for (var j = 0; j < le - 1; j++) {
          if (j == 0)
              type3data = (Array(2).join(0) + (bytes[1 + (5*i) + j].toString(16).toUpperCase())).slice(-2);
          else
              type3data = type3data + (Array(2).join(0) + (bytes[1 + (5*i) + j].toString(16).toUpperCase())).slice(-2);
      }
