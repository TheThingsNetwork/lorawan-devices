...
function decodeUplink(input) {
  ...
  switch (input.fPort) {
    case 1:
      data.occupied = input.bytes[0] & 0x80;
      data.keepAlive = input.bytes[0] & 0x01;
      data.reset = input.bytes[0] & 0x02;
      data.No_Beacon = input.bytes[0] & 0x04;
      data.Radar = input.bytes[0] & 0x08;
      data.Obstruction = input.bytes[0] & 0x10;
      data.Good_Battery = input.bytes[0] & 0x20;
      ...
      if (input.bytes.length > 3) {
        ...
        for (let index = 4; index < input.bytes.length; index++) {
...
function decodeDownlink(input) {
  ...
  switch (input.fPort) {
    case 0:
      ...
      downlinkdata.radar_start_range = input.bytes[0] & 0x70;
      downlinkdata.suppress_baseline_offset = input.bytes[0] & 0x80;

      downlinkdata.deflection_threshold_mag = input.bytes[1] & 0x0F;
      downlinkdata.debug_mode = input.bytes[1] & 0x10;
      downlinkdata.axis_deflection = input.bytes[1] & 0x60;
      downlinkdata.downlink_frequency = input.bytes[1] & 0x80;

      downlinkdata.fluctuation_threshold_XY = input.bytes[2] & 0x07;
      downlinkdata.hw_acc_samples_no = input.bytes[2] & 0x18;
      downlinkdata.oversampling = input.bytes[2] & 0x60;
      downlinkdata.zero_neg_drop = input.bytes[2] & 0x80;

      downlinkdata.fluctuation_delay_time = input.bytes[3] & 0x07;
      downlinkdata.mag_reboot = input.bytes[3] & 0x08;
      downlinkdata.reset_control = input.bytes[3] & 0x10;
      downlinkdata.sleep_time = input.bytes[3] & 0xE0;

      downlinkdata.keepalive_period = input.bytes[4] & 0x03;
      downlinkdata.self_heal = input.bytes[4] & 0x04;
      downlinkdata.fluctuation_threshold_Z = input.bytes[4] & 0x38;
      downlinkdata.axis_fluctuations = input.bytes[4] & 0xC0;

      downlinkdata.reflection_threshold_radar = input.bytes[5] & 0x07;
      downlinkdata.obstruction_check = input.bytes[5] & 0x08;
      downlinkdata.radar_scan_count = input.bytes[5] & 0xF0;

      downlinkdata.ble_scan_window = input.bytes[6] & 0x07;
      downlinkdata.beacons_scan_no = input.bytes[6] & 0x38;
      downlinkdata.send_rssi = input.bytes[6] & 0x40;
      downlinkdata.OI = input.bytes[6] & 0x80;
