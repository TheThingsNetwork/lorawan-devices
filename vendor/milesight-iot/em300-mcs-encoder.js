...
function milesightDeviceEncode(payload) {
    ...
    if ("magnet_alarm_config" in payload) {
        encoded = encoded.concat(setMagnetAlarm(payload.magnet_alarm_config));
    }
...
/**
 * set magnet status alarm
 * @param {object} magnet_alarm_config
 * @param {number} magnet_alarm_config.enable values: (0: disable, 1: enable)
 * @param {number} magnet_alarm_config.report_interval unit: second
 * @param {number} magnet_alarm_config.report_times
 * @example { "magnet_alarm_config": { "enable": 1, "report_interval": 600, "report_times": 10 } }
 */
function setMagnetAlarm(magnet_alarm_config) {
    var enable = magnet_alarm_config.enable;
    var report_interval = magnet_alarm_config.report_interval;
    var report_times = magnet_alarm_config.report_times;
 ...
 * @param {number} d2d_master_config.mode values: (1: temperature_alarm, 2: temperature_alarm_release, 3: magnet_status_alarm, 4: magnet_status_alarm_release)
...
function setD2DMasterConfig(d2d_master_config) {
    ...
    var report_type_map = { 0: "lora", 1: "d2d", 3: "d2d_and_lora" };
    var report_type_values = getValues(report_type_map);
    if (report_type_values.indexOf(report_type) === -1) {
        throw new Error("d2d_master_config._item.report_type must be one of " + report_type_values.join(", "));
    }
