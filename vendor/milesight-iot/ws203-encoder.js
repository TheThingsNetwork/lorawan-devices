...
/**
 * set vacant reporting interval
 * @param {number} vacant_reporting_interval unit: second, range: [60, 3600]
 * @example { "vacant_reporting_interval": 300 }
 */
function setVacantReportingInterval(vacant_reporting_interval) {
    if (vacant_reporting_interval < 60 || vacant_reporting_interval > 3600) {
        throw new Error("vacant_reporting_interval must be between 60 and 3600");
    }

    var buffer = new Buffer(4);
    buffer.writeUInt8(0xff);
    buffer.writeUInt8(0x95);
    buffer.writeUInt16LE(vacant_reporting_interval);
    return buffer.toBytes();
}
