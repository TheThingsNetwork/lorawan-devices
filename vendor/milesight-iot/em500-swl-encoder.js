 ...
 * @param {object} depth_alarm_config
 * @param {number} depth_alarm_config.enable values: (0: disable, 1: enable)
 * @param {number} depth_alarm_config.condition values: (0: disable, 1: below, 2: above, 3: between, 4: outside)
 * @param {number} depth_alarm_config.threshold_min condition=(below, within, outside)
 * @param {number} depth_alarm_config.threshold_max condition=(above, within, outside)
 */
function setDepthAlarmConfig(depth_alarm_config) {
    var condition = depth_alarm_config.condition;
    var enable = depth_alarm_config.enable;
    var threshold_min = depth_alarm_config.threshold_min;
    var threshold_max = depth_alarm_config.threshold_max;

    var condition_map = { 0: "disable", 1: "below", 2: "above", 3: "between", 4: "outside" };
    var condition_values = getValues(condition_map);
    if (condition_values.indexOf(condition) === -1) {
        throw new Error("depth_alarm_config.condition must be one of " + condition_values.join(", "));
    }
    var enable_map = { 0: "disable", 1: "enable" };
    var enable_values = getValues(enable_map);
    if (enable_values.indexOf(enable) === -1) {
        throw new Error("depth_alarm_config.enable must be one of " + enable_values.join(", "));
    }
