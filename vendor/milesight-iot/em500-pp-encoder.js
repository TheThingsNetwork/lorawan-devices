...
function milesightDeviceEncode(payload) {
    ...
    if ("pressure_alarm_config" in payload) {
        encoded = encoded.concat(setPressureAlarmConfig(payload.pressure_alarm_config));
    }
    ...
    if ("pressure_calibration_settings" in payload) {
        encoded = encoded.concat(setPressureCalibrationConfig(payload.pressure_calibration_settings));
    }
...
/**
 * pressure alarm configuration
 * @param {object} pressure_alarm_config
 * @param {number} pressure_alarm_config.enable values: (0: disable, 1: enable)
 * @param {number} pressure_alarm_config.condition values: (0: disable, 1: below, 2: above, 3: between, 4: outside)
 * @param {number} pressure_alarm_config.threshold_min condition=(below, within, outside)
 * @param {number} pressure_alarm_config.threshold_max condition=(above, within, outside)
 * @param {number} pressure_alarm_config.alarm_release_enable values: (0: disable, 1: enable)
 */
function setPressureAlarmConfig(pressure_alarm_config) {
    var condition = pressure_alarm_config.condition;
    var enable = pressure_alarm_config.enable;
    var threshold_min = pressure_alarm_config.threshold_min;
    var threshold_max = pressure_alarm_config.threshold_max;
    var alarm_release_enable = pressure_alarm_config.alarm_release_enable;

    var condition_map = { 0: "disable", 1: "below", 2: "above", 3: "between", 4: "outside" };
    var condition_values = getValues(condition_map);
    if (condition_values.indexOf(condition) === -1) {
        throw new Error("pressure_alarm_config.condition must be one of " + condition_values.join(", "));
    }
    var enable_map = { 0: "disable", 1: "enable" };
    var enable_values = getValues(enable_map);
    if (enable_values.indexOf(enable) === -1) {
        throw new Error("pressure_alarm_config.enable must be one of " + enable_values.join(", "));
    }
    if (enable_values.indexOf(alarm_release_enable) === -1) {
        throw new Error("pressure_alarm_config.alarm_release_enable must be one of " + enable_values.join(", "));
    }

    var data = 0x00;
    data |= getValue(condition_map, condition);
    data |= 1 << 3; // pressure alarm
 ...
 * @example { "pressure_calibration_settings": { "enable": 1, "calibration_value": 23 } }
 */
function setPressureCalibrationConfig(pressure_calibration_settings) {
    ...
    buffer.writeUInt8(0x03); // pressure
