...
function decode(parameters, bytes, port, flat) {
  ...
  while (bytes.length > 0) {
    ...
    if (!parameters[port].hasOwnProperty(header)) {
      return { errors: ["Couldn't find header " + header + ' in decoder object.' + ' Are you decoding the correct sensor?'] };
    }

    var properties = parameters[port][header];

    if (properties.length === 0) {
      return { errors: ['Something is wrong with the decoder object. Check ' + 'port ' + port + ', header ' + header + ''] };
    }
