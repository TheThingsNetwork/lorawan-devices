...
function encode_write_field(command, lookup, encoded_data) {
  ...
  var written_bits = write_bits(value, parseInt(lookup['bit_start']), parseInt(lookup['bit_end']), lookup['type'], 0);
...
function encode_write_group(commands, group_lookup, encoded_data) {
  ...
  for (var i = 0; i < field_names.length; i++) {
    ...
    if (lookup['multiple'] == 0 || lookup['multiple'] === undefined) {
      written_bits = write_bits(value, parseInt(lookup['bit_start']), parseInt(lookup['bit_end']), lookup['type'], written_bits);
    } else {
...
function encode(commands, sensor) {
  ...
  var categories = Object.keys(commands);
  for (var i = 0; i < categories.length; i++) {
    // iterates over the categories of commands
    ...
    for (var j = 0; j < groups_and_fields.length; j++) {
      // iterates over the groups of commands
