function PrintHex(bytes, offset, length) {
  ...
  for (var i = offset; i < offset + length; i++) {
    ...
    if (output.length === 0) output = output + s;
...
function DecodeSatId(id) {
  if (id < 32) return 'G' + (id + 1);
  else if (id >= 32 && id <= 50) return 'S' + (id + 88);
  else if (id >= 64 && id <= 124) return 'B' + (id - 63);
  else return '?' + id;
}
...
function ParseNav1(b) {
  ...
  while (b.bitLength - b.offset >= 8) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (fHasPsuedoRange) sat.psuedoRangeNs = b.U32LE(19) * 3;
      if (fHas19BitC) sat.c19 = b.U32LE(19);
      if (fHasDoppler) sat.dopplerHz = b.S32LE(15);
      if (fBadDoppler) sat.fBadDoppler = true;
      if (fHasBitChange) sat.bitChange = b.U32LE(8);
      if (fHas8BitA) sat.a8 = b.U32LE(8);

      out.sats[out.sats.length] = sat;
    }
  }
  if (b.U32LE(b.bitLength - b.offset) != 0) throw 'Non-zero padding!';
...
function ParseNav2(b) {
  ...
  try {
    ...
    for (var scan = 0; scan < 2 && b.bitLength - b.offset >= 8; scan++) {
      ...
      if (scan === 0 && fHasBitChange) out.bitChange = b.U32LE(25).toString(16);

      var cnt = b.U32LE(4);
      var fShortId = b.U32LE(1) !== 0;

      var dopCnt = 0;
      if (fHasDoppler) dopCnt = b.U32LE(3);

      if (scan !== 0) out.gap = b.U32LE(6).toString(16);

      for (var i = 0; i < cnt; i++) {
        var sat = {};

        if (scanTypes == 0) sat.id = 'G';
        else sat.id = 'B';

        if (fShortId) sat.id += (b.U32LE(5) + 1).toString();
        else sat.id += (b.U32LE(6) + 1).toString();

        sat.cno = 45 - 4 * b.U32LE(2);

        if (i == 0) sat.psuedoRangeNs = 0;
        else sat.psuedoRangeNs = b.U32LE(18) * 4;
    ...
    if (b.bitLength - b.offset >= 8) throw 'Extra scan?!';
    if (b.U32LE(b.bitLength - b.offset) != 0) throw 'Non-zero padding!';
  } catch {
...
function ParseNav(bytes) {
  ...
  if (frameType === 1) return ParseNav1(b);
  else if (frameType === 2) return ParseNav2(b);
  else return null;
}
...
function decodeUplink(input) {
  ...
  if (p === 1) {
    ...
    d.percentOther = 100 - d.percentLora - d.percentGnss - d.percentWifi - d.percentSleep - d.percentDisch;
  } else if (p === 5) {
    d._type = 'location';
    wifiCount = b.U32LE(5);
