function decodeUplink(input) {
  ...
  if (port === 1) {
    ...
    location.latitudeDeg = bytes[0] + bytes[1] * 256 + bytes[2] * 65536 + bytes[3] * 16777216;
    if (location.latitudeDeg >= 0x80000000) {
      // 2^31
      location.latitudeDeg -= 0x100000000; // 2^32
    }
    location.latitudeDeg /= 1e7;

    location.longitudeDeg = bytes[4] + bytes[5] * 256 + bytes[6] * 65536 + bytes[7] * 16777216;
    if (location.longitudeDeg >= 0x80000000) {
      // 2^31
    ...
    location.latitudeDeg = bytes[0] + bytes[1] * 256 + bytes[2] * 65536;
    if (location.latitudeDeg >= 0x800000) {
      // 2^23
      location.latitudeDeg -= 0x1000000; // 2^24
    }
    location.latitudeDeg *= 256e-7;

    location.longitudeDeg = bytes[3] + bytes[4] * 256 + bytes[5] * 65536;
    if (location.longitudeDeg >= 0x800000) {
      // 2^23
    ...
    decoded.initialBatV = Number((4.0 + 0.1 * (bytes[0] & 0xf)).toFixed(1));
