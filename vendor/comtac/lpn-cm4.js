    ...
    for (i = 4; i < data.length; i++) {
      ...
      switch (data[i]) {
        ...
        case TYPE_TEMP_HUM_HISTORY: //Temp/Hum history
          ...
          obj.tempNOW = [tempNOW_0, tempNOW_1, tempNOW_2, tempNOW_3, tempNOW_4, tempNOW_5, tempNOW_6, tempNOW_7];
          obj.humNOW = [humNOW_0, humNOW_1, humNOW_2, humNOW_3, humNOW_4, humNOW_5, humNOW_6, humNOW_7];
          i += 24;
          break;
...
function encodeDownlink(input) {
  if (input.data.fPort == CONFIG_PORT) {
    if (
      typeof input.data.measRate !== 'undefined' &&
      ...
      return {
        fPort: CONFIG_PORT,
        bytes: [
          (input.data.measRate >> 8) & 0xff,
          input.data.measRate & 0xff,
          input.data.historyTrigger,
          (input.data.tempOffset >> 8) & 0xff,
          input.data.tempOffset & 0xff,
          tempMax_raw,
          tempMin_raw,
          input.data.humOffset,
          humMax_raw,
          humMin_raw,
        ],
      };
