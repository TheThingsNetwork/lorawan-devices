...
var POS_REQUESTS = ['INVALID', 'GPS ONLY', 'WIFI ONLY', 'GPS OR WIFI', 'WIFI OR GPS', 'GPS AND WIFI'];

//HELPING FUNCTIONS
function bin32dec(bin) {
  var num = bin & 0xffffffff;
  if (0x80000000 & num) num = -(0x0100000000 - num);
  return num;
}
      ...
      switch (data[i]) {
        ...
        case TYPE_WIFI_DATA:
          ...
          switch (nrAPs) {
            case 4:
              var MAC1 =
                bin8string(data[i + 2]) + ':' + bin8string(data[i + 3]) + ':' + bin8string(data[i + 4]) + ':' + bin8string(data[i + 5]) + ':' + bin8string(data[i + 6]) + ':' + bin8string(data[i + 7]);
              var RSSI1 = data[i + 8] * -1;
              var MAC2 =
                bin8string(data[i + 9]) +
                ':' +
                bin8string(data[i + 10]) +
                ':' +
                bin8string(data[i + 11]) +
                ':' +
                bin8string(data[i + 12]) +
                ':' +
                bin8string(data[i + 13]) +
                ':' +
                bin8string(data[i + 14]);
              var RSSI2 = data[i + 15] * -1;
              var MAC3 =
                bin8string(data[i + 16]) +
                ':' +
                bin8string(data[i + 17]) +
                ':' +
                bin8string(data[i + 18]) +
                ':' +
                bin8string(data[i + 19]) +
                ':' +
                bin8string(data[i + 20]) +
                ':' +
                bin8string(data[i + 21]);
              var RSSI3 = data[i + 22] * -1;
              var MAC4 =
                bin8string(data[i + 23]) +
                ':' +
                bin8string(data[i + 24]) +
                ':' +
                bin8string(data[i + 25]) +
                ':' +
                bin8string(data[i + 26]) +
                ':' +
                bin8string(data[i + 27]) +
                ':' +
                bin8string(data[i + 28]);
...
function encodeDownlink(input) {
  if (input.data.fPort == DATA_PORT) {
    ...
    if (typeof input.data.configGet !== 'undefined') {
      ...
      typeof input.data.pingInterval !== 'undefined' &&
      ...
      return {
        fPort: CONFIG_PORT,
        bytes: [
          (input.data.pingInterval >> 8) & 0xff,
          input.data.pingInterval & 0xff,
          input.data.longRangeTrigger,
          input.data.midRangeTrigger,
          (input.data.rejoinTrigger >> 8) & 0xff,
          input.data.rejoinTrigger & 0xff,
          input.data.gpsFixes,
          input.data.minWIFIDetects,
        ],
      };
    } else {
