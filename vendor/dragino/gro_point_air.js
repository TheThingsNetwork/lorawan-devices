function decodeUplink(input) {
  ...
  var tem = ['tem1', 'tem2', 'tem3', 'tem4', 'tem5', 'tem6', 'tem7', 'tem8', 'tem9', 'tem10', 'tem11', 'tem12', 'tem13'];
  ...
  if (is_tem_flag === true) {
    for (i = 0; i < tem_len[sen_type]; i++) {
      ...
      else dic[tem[i]] = ((bytes[start_bytes] << 8) | bytes[start_bytes + 1]) / 10.0;
  ...
  for (i = 0; i < hum_len[sen_type]; i++) {
    if (bytes[start_bytes] == 0x7f && bytes[start_bytes + 1] == 0xff) {
      if (is_tem_flag === true) dic[hum[1]] = 'Invalid data';
      else {
