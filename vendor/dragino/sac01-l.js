function decodeUplink(input) {
  ...
  switch (input.fPort) {
    case 2:
      ...
      data.mod = bytes[2];
      data.int = (bytes[7] << 24) | (bytes[8] << 16) | (bytes[9] << 8) | bytes[10];
      data.out = (bytes[3] << 24) | (bytes[4] << 16) | (bytes[5] << 8) | bytes[6];
      return {
        data: data,
      };
      break;
    ...
    case 4:
      ...
      if (dataTimes.some((time) => time !== null)) {
        ...
        data.Data_Times = dataTimes.filter((time) => time !== '1970-01-01 00:00:00');
      }
      if (readings.some((reading) => reading !== 0)) {
        data.Readings = readings.filter((reading) => reading !== 0);
      }
      return {
        data: data,
      };
    ...
    case 5:
      {
        if (bytes[0] == 0x38) data.SENSOR_MODEL = 'SAC01-L';
