...
function getDeviceType(devName) {
  if (devName == 'R718B2') return 20;
  else if (devName == 'R718CK2') return 22;
  else if (devName == 'R718CT2') return 23;
}
...
function decodeUplink(input) {
  ...
  switch (input.fPort) {
    case 6:
      ...
      } else data.Temp1 = ((input.bytes[4] << 8) | input.bytes[5]) / 10;

      if (input.bytes[6] & 0x80) {
        var tmpval2 = (input.bytes[6] << 8) | input.bytes[7];
        data.Temp2 = ((0x10000 - tmpval2) / 10) * -1;
      } else data.Temp2 = ((input.bytes[6] << 8) | input.bytes[7]) / 10;
