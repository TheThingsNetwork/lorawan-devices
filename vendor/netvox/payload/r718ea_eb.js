...
function getCmdToID(cmdtype) {
  ...
  else if (cmdtype == 'SetActiveThresholdReq') return 3;
  else if (cmdtype == 'SetActiveThresholdRsp') return 131;
  else if (cmdtype == 'GetActiveThresholdReq') return 4;
  else if (cmdtype == 'GetActiveThresholdRsp') return 132;
}

function getDeviceID(devName) {
  if (devName == 'R718EA') return 154;
  else if (devName == 'R718EB') return 200;
}
  ...
  switch (input.fPort) {
    case 6:
      ...
      if (input.bytes[1] === 0x9a) {
        if (input.bytes[4] & 0x80) data.AngleX = (0x100 - input.bytes[4]) * -1;
        else data.AngleX = input.bytes[4];

        if (input.bytes[5] & 0x80) data.AngleY = (0x100 - input.bytes[5]) * -1;
        else data.AngleY = input.bytes[5];

        if (input.bytes[6] & 0x80) data.AngleZ = (0x100 - input.bytes[6]) * -1;
        else data.AngleZ = input.bytes[6];

        if (input.bytes[7] & 0x80) {
          var tmpval = (input.bytes[7] << 8) | input.bytes[8];
          data.Temp = ((0x10000 - tmpval) / 10) * -1;
        } else data.Temp = ((input.bytes[9] << 8) | input.bytes[10]) / 10;
      } else if (input.bytes[1] === 0xc8) {
        if (input.bytes[4] & 0x80) {
          var tmpval = (input.bytes[4] << 8) | input.bytes[5];
          data.AngleX = ((0x10000 - tmpval) / 10) * -1;
        } else data.AngleX = ((input.bytes[4] << 8) | input.bytes[5]) / 10;

        if (input.bytes[6] & 0x80) {
          var tmpval = (input.bytes[6] << 8) | input.bytes[7];
          data.AngleY = ((0x10000 - tmpval) / 10) * -1;
        } else data.AngleY = ((input.bytes[6] << 8) | input.bytes[7]) / 10;

        if (input.bytes[8] & 0x80) {
          var tmpval = (input.bytes[8] << 8) | input.bytes[9];
          data.AngleZ = ((0x10000 - tmpval) / 10) * -1;
        } else data.AngleZ = ((input.bytes[8] << 8) | input.bytes[9]) / 10;
      }
    ...
    case 7:
      ...
      if (input.bytes[0] === getCmdToID('ConfigReportRsp')) {
        ...
        } else if (input.bytes[1] === 0xc8) data.AngleChange = ((input.bytes[7] << 8) | input.bytes[8]) / 10;
