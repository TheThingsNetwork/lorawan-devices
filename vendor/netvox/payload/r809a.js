...
function getCmdToID(cmdtype) {
  ...
  else if (cmdtype == 'ClearEnergy') return 147;
  else if (cmdtype == 'ReadCurrentStatus') return 148;
}

function getDeviceID(devName) {
  if (devName == 'R809A') return 14;
  else if (devName == 'R816B') return 115;
}
...
function encodeDownlink(input) {
  ...
  if (input.data.Cmd == 'ConfigReportReq') {
    ...
    input.data.Cmd == 'ReadConfigReportReq' ||
    input.data.Cmd == 'On' ||
    input.data.Cmd == 'Off' ||
    input.data.Cmd == 'Toggle' ||
    input.data.Cmd == 'ClearEnergy' ||
    input.data.Cmd == 'ReadCurrentStatus'
  ) {
