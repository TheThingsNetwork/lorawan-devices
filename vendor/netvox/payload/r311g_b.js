...
function getDeviceID(devName) {
  if (devName == 'R311G') return 4;
  else if (devName == 'R311B') return 75;
}
...
function encodeDownlink(input) {
  ...
  if (input.data.Cmd == 'ConfigReportReq') {
    ...
    if (input.data.Device == 'R311B') tmpVal = input.data.IlluminanceThreshold;
    else if (input.data.Device == 'R311G') tmpVal = input.data.IlluminanceChange;
  ...
  switch (input.fPort) {
    case 7:
      ...
      if (input.bytes[0] === getCmdToID('ConfigReportReq')) {
        ...
        data.MaxTime = (input.bytes[4] << 8) | input.bytes[5];
        data.BatteryChange = input.bytes[6] / 10;
        if (data.Device == 'R311G') data.IlluminanceChange = (input.bytes[7] << 8) | input.bytes[8];
        else if (data.Device == 'R311B') data.IlluminanceThreshold = (input.bytes[7] << 8) | input.bytes[8];
      }
