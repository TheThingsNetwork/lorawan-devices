...
function decodeUplink(input) {
  ...
  switch (input.fPort) {
    case 6:
		...
		data.Current = input.bytes[5];
		data.ADCRaw = (input.bytes[6] << 8 | input.bytes[7]);
		data.FineCurrent = input.bytes[8]/10;
	...
	case 7:
		...
		else if (input.bytes[0] === 0x82)
		{
			...
			data.CurrentChange = input.bytes[9];
		}
		break;
...
function encodeDownlink(input) {
  ...
  if (input.data.Cmd == "ConfigReportReq")
  {
	  ...
	  port = 7;
	  ret = ret.concat(getCmdID, devid, (mint >> 8), (mint & 0xFF), (maxt >> 8), (maxt & 0xFF), batteryChg, (adcChg >> 8), (adcChg & 0xFF), currentChg, 0x00);
  }
