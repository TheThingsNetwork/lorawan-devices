...
function getDeviceType(devName) {
  if (devName == 'R718MBB') return 43;
  else if (devName == 'R311FB') return 80;
  else if (devName == 'R730MBB') return 131;
}
...
function decodeUplink(input) {
  ...
  switch (input.fPort) {
    case 6:
      ...
      data.WorkCount = (input.bytes[4] << 24) | (input.bytes[5] << 16) | (input.bytes[6] << 8) | input.bytes[7];
