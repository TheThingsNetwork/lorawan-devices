...
function decodeUplink(input) {
  ...
  switch (input.fPort) {
    case 22:
		...
		data.LowCurrent1Alarm = input.bytes[14] & 1;
		data.HighCurrent1Alarm = input.bytes[14]>>1 & 1;
		data.LowCurrent2Alarm = input.bytes[14]>>2 & 1;
		data.HighCurrent2Alarm = input.bytes[14]>>3 & 1;
		data.LowCurrent3Alarm = input.bytes[14]>>4 & 1;
		data.HighCurrent3Alarm = input.bytes[14]>>5 & 1;

		if(input.bytes[15] === 0x00){
			data.ShockTamperAlarm = 'NoAlarm';
		}else if(input.bytes[15] === 0x01){
