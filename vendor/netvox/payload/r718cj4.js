...
function decodeUplink(input) {
  ...
  switch (input.fPort) {
    case 6:
		...
		// Process temperature values based on report type
		if (input.bytes[2] === 0x01) {
			...
			// Temperature3 and Temperature4
			if (input.bytes[4] & 0x80)
			{
				var tmpval1 = (input.bytes[4]<<8 | input.bytes[5]);
				data.Temperature3 = (0x10000 - tmpval1)/10 * -1;
			}
			else
				data.Temperature3 = (input.bytes[4]<<8 | input.bytes[5])/10;

			if (input.bytes[6] & 0x80)
			{
				var tmpval2 = (input.bytes[6]<<8 | input.bytes[7]);
				data.Temperature4 = (0x10000 - tmpval2)/10 * -1;
			}
			else
				data.Temperature4 = (input.bytes[6]<<8 | input.bytes[7])/10;
		}
