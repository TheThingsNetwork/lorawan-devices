...
function decodeUplink(input) {
  ...
  switch (input.fPort) {
    case 6:
      ...
      data.AirPressure = ((input.bytes[4] << 32) | (input.bytes[5] << 16) | (input.bytes[6] << 8) | input.bytes[7]) / 100;
      if (input.bytes[8] & 0x80) {
        var tmpval = (input.bytes[8] << 8) | input.bytes[9];
        data.Temp = ((0x10000 - tmpval) / 100) * -1;
      } else data.Temp = ((input.bytes[8] << 8) | input.bytes[9]) / 100;
    ...
    case 7:
      ...
      if (input.bytes[0] === getCmdToID('ConfigReportRsp')) {
        ...
        data.AirPressureChange = ((input.bytes[9] << 8) | input.bytes[10]) / 10;
      }
...
function encodeDownlink(input) {
  ...
  if (input.data.Cmd == 'ConfigReportReq') {
    ...
    var airChg = input.data.AirPressureChange * 10;

    ret = ret.concat(getCmdID, devid, mint >> 8, mint & 0xff, maxt >> 8, maxt & 0xff, batteryChg, tempChg >> 8, tempChg & 0xff, airChg >> 8, airChg & 0xff);
