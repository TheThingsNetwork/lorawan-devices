...
function getCmdToID(cmdtype) {
  ...
  else if (cmdtype == 'SetOnDistanceThresholdReq') return 3;
  else if (cmdtype == 'SetOnDistanceThresholdRsp') return 131;
  else if (cmdtype == 'GetOnDistanceThresholdReq') return 4;
  else if (cmdtype == 'GetOnDistanceThresholdRsp') return 132;
  else if (cmdtype == 'SetFillMaxDistanceReq') return 5;
  else if (cmdtype == 'SetFillMaxDistanceRsp') return 133;
  else if (cmdtype == 'GetFillMaxDistanceReq') return 6;
  else if (cmdtype == 'GetFillMaxDistanceRsp') return 134;
  else if (cmdtype == 'SetDeadZoneDistanceReq') return 11;
  else if (cmdtype == 'SetDeadZoneDistanceRsp') return 139;
  else if (cmdtype == 'GetDeadZoneDistanceReq') return 12;
  else if (cmdtype == 'GetDeadZoneDistanceRsp') return 140;
}
  ...
  switch (input.fPort) {
    case 6:
      ...
      if (input.bytes[1] === 0x34) {
        ...
        } else data.Temp = ((input.bytes[7] << 8) | input.bytes[8]) / 10;
    ...
    case 7:
      ...
      data.Device = getDeviceName(input.bytes[1]);
      if (
        input.bytes[0] === getCmdToID('ConfigReportRsp') ||
        input.bytes[0] === getCmdToID('SetOnDistanceThresholdRsp') ||
        ...
        if (input.bytes[1] === 0x34) data.TempChange = ((input.bytes[9] << 8) | input.bytes[10]) / 10;
...
function encodeDownlink(input) {
  ...
  if (input.data.Cmd == 'ConfigReportReq') {
    ...
    } else if (devid == 0xb1) ret = ret.concat(getCmdID, devid, mint >> 8, mint & 0xff, maxt >> 8, maxt & 0xff, batteryChg, disChg >> 8, disChg & 0xff, 0x00, 0x00);
