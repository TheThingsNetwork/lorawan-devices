...
function decodeUplink(input) {
  ...
  if (input.fPort == 1 || input.fPort == 2) {
    var hum = (bytes[0] << 8) | bytes[1];
    hum = 0.1 * hum;
    hum = Number(hum.toFixed(1));
    var tem = ((bytes[2] << 24) >> 16) | bytes[3];
    tem = 0.1 * tem;
    tem = Number(tem.toFixed(1));
    var rpm = (bytes[4] << 8) | bytes[5];
    rpm = 0.1 * rpm;
    rpm = Number(rpm.toFixed(0));
    var fpm = (bytes[6] << 8) | bytes[7];
    fpm = 0.1 * fpm;
    fpm = Number(fpm.toFixed(0));
  ...
  // firmware version 1.5
  if (input.fPort == 2) {
    var pre = (bytes[8] << 8) | bytes[9];
    pre = Number(pre.toFixed(0));
    var co = (bytes[10] << 8) | bytes[11];
    co = 1.0 * co;
    co = Number(co.toFixed(0));
    var nh3 = (bytes[12] << 8) | bytes[13];
    nh3 = 1.0 * nh3;
    nh3 = Number(nh3.toFixed(0));
    var no2 = (bytes[14] << 8) | bytes[15];
    no2 = 0.01 * no2;
    no2 = Number(no2.toFixed(2));
    var noise = (bytes[16] << 8) | bytes[17];
    noise = 0.01 * noise;
  ...
  if (input.fPort == 3) {
    ...
    tem = 0.1 * tem;
    tem = Number(tem.toFixed(1));
    var pre = (bytes[4] << 8) | bytes[5];
    pre = Number(pre.toFixed(0));

    var pm1 = (bytes[6] << 8) | bytes[7];
    pm1 = 0.1 * pm1;
    pm1 = Number(pm1.toFixed(0));
    var pm25 = (bytes[8] << 8) | bytes[9];
    pm25 = 0.1 * pm25;
    pm25 = Number(pm25.toFixed(0));
    var pm10 = (bytes[10] << 8) | bytes[11];
    pm10 = 0.1 * pm10;
    pm10 = Number(pm10.toFixed(0));

    var noise = (bytes[12] << 8) | bytes[13];
    noise = 0.01 * noise;
    noise = Number(noise.toFixed(2));
