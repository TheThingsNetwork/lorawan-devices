...
var translation = {
  ...
  pm2: "[10] PM2.5 [µg/m³]",
  pm10: "[11] PM10 [µg/m³]",
  trigger: "[20] Thresholds Triggered",
  stop: "[21] Thresholds Stopped",
  state: "[22] Switch State",
  state_val: {
    0: "Off",
    1: "On",
  },
  ...
  lat: "[23] Latitude",
  lon: "[24] Longitude",
  info: "[25] Firmware Info",
  ccrc: "[26] Config CRC",
  cfrom: "[27] Config from position",
  cto: "[28] Config to position",
  config: "[29] Configuration",
  ...
  stdel: "[26] LoRaButton Transmit first heartbeat/measurement/reminder (+/- 25%) [sec]",
  ...
  tbuztog: "[33] LoRaButton Feedback $ buzzer toggle interval [sec]",
  tbuzdur: "[34] LoRaButton Feedback $ buzzer duration [sec]",
  tbuzrem: "[35] LoRaButton Feedback $ buzzer reminder interval [sec]",
  tledpin: "[36] LoRaButton Feedback $ LED pin",
  tledpin_val: {
    255: "No pin",
    128: "Mix LED 1",
    129: "Mix LED 2",
  ...
  tledtog: "[37] LoRaButton Feedback $ LED toggle interval [sec]",
  tleddur: "[38] LoRaButton Feedback $ LED duration [sec]",
  tledrem: "[39] LoRaButton Feedback $ LED reminder interval [sec]",
  subband: "[40] LoRaWAN Sub Band for US915 and AU915 (1-8)",
  subband_val: {
    0: "Try all subbands",
  },
  ...
  succcnt: "[31] Update Success count",
  scd30result: "[32] SCD30 Result",
  scd30result_val: {
    0: "Success",
    1: "Start Timeout",
    2: "Address Timeout",
    ...
    48: "Data NACK",
    56: "Arbitration lost",
    241: "Recalibration pending",
    242: "Recalibration canceled",
    243: "Recalibration missing argument",
    245: "No Auto Calibration on batteries",
    255: "Invalid command/argument length",
  },
  alarmid: "[33] Alarm ID",
  feedbacktime: "[34] Feedback Time [min]",
  feedbacks: "[35] Feedbacks",
  uiderror: "[36] UID Error",
  uiderror_val: {
  ...
  error_val: {
    ...
    146: "Unexpected end, measurements without (complete) Particulate Matter",
...
function MutelcorLoRaButtonDecode(bytes, port) {
  ...
  if (decoded.opcode === 3 || decoded.opcode === 5) {
    ...
    if (measurements & 1) {
      ...
      decoded.meas.temp = bytes[pos++] * 256 + bytes[pos++];
      decoded.meas.temp <<= 16;  // Temperature is 16 bit signed, javascript 32 bit signed, move the sign bit to the right position
      decoded.meas.temp /= 655360; // Get the signed temperature back to Celcius value
    ...
    if (measurements & 128) {
      ...
      if (measurements & 2) {
        ...
        decoded.meas.pm1 = bytes[pos++] * 256 + bytes[pos++];
        decoded.meas.pm2 = bytes[pos++] * 256 + bytes[pos++];
        decoded.meas.pm10 = bytes[pos++] * 256 + bytes[pos++];
      }
    }
    if (decoded.opcode === 5) {
      if (bytes.length < pos + 1) {
  ...
  if (decoded.opcode === 113 || decoded.opcode === 114) {
    ...
    if (decoded.opcode === 113) {
      ...
      while (config_index < config_end) {
        ...
        if (194 <= config_index && config_index <= 197) config_pos = 194;
