...
function Heartbeat_proc(bytes) {
  ...
  //Temperature
  if (0 == ((bytes[5] >> 7) & 0x01)) {
      Heartbeat_Msg.temperature = ((bytes[5] & 0x7F) + (0.01 * bytes[6])) + "℃";
  } else {
      Heartbeat_Msg.temperature = ((-1) * ((bytes[5] & 0x7F) + (0.01 * bytes[6]))) + "℃";
  }

  //humidity
  Heartbeat_Msg.humidity = bytes[7] + "%";
  //CRC
  Heartbeat_Msg.crc = (((bytes[8] << 8) & 0xFF00) | (bytes[9] & 0xFF));
