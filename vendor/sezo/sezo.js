...
function decodeUplink(input) {
  while (i < input.bytes.length) {
    ...
    var s_no = input.bytes[i++];
    var s_type = input.bytes[i++];
    if (typeof sensor_types[s_type] == 'undefined') throw 'Sensor type error!: ' + s_type;
    ...
    switch (s_type) {
      ...
      case 211: // External Pressure
        var s_value = arrayToDecimal(bytes.slice(i, i + s_size), (is_signed = sensor_types[s_type].signed), (divisor = sensor_types[s_type].divisor));
        break;
