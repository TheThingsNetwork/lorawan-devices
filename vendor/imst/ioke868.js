      ...
      case OBJECT_TYPE:
        ...
        if (retArray.length >= 3) {
          ...
          if (retArray.length >= 6) {
            ...
            if ((ascaler & 0x80) == 0x80) ascaler -= 0x100;
            //mantissa
            value = retArray[5];
...
function decodeMeterID(payload, pos) {
  ...
  if (payload.length >= pos + METER_FIELD_HEADER_SIZE) {
    ...
    if (payload.length >= pos + length) {
      ...
      else meterIDString = buffer_to_hex_string(meterID);
...
function decodeObjectWithIDAndUnitField(payload, pos) {
  return decodeObject(
    payload,
    pos,
    OBJECT_ID_AND_UNIT_FIELD_HEADER_SIZE,
    OBJECT_ID_AND_UNIT_FIELD_TYPE_INDEX,
    OBJECT_ID_AND_UNIT_FIELD_LENGTH_INDEX,
    OBJECT_ID_AND_UNIT_FIELD_OBIS_ID_INDEX,
    OBJECT_ID_AND_UNIT_FIELD_GROUP_MASK_INDEX,
    OBJECT_ID_AND_UNIT_FIELD_UNIT_INDEX_DEPENDEND_OBIS_ID,
    OBJECT_ID_AND_UNIT_FIELD_SCALER_INDEX_DEPENDEND_OBIS_ID,
    OBJECT_ID_AND_UNIT_FIELD_MANTISSA_INDEX_DEPENDEND_OBIS_ID
  );
}
...
function decodeObjectWithIDField(payload, pos) {
  return decodeObject(
    payload,
    pos,
    OBJECT_WITH_ID_FIELD_HEADER_SIZE,
    OBJECT_WITH_ID_FIELD_TYPE_INDEX,
    OBJECT_WITH_ID_FIELD_LENGTH_INDEX,
    OBJECT_WITH_ID_FIELD_OBIS_ID_INDEX,
    OBJECT_WITH_ID_FIELD_GROUP_MASK_INDEX,
    -1,
    OBJECT_WITH_ID_FIELD_SCALER_INDEX_DEPENDEND_OBIS_ID,
    OBJECT_WITH_ID_FIELD_MANTISSA_INDEX_DEPENDEND_OBIS_ID
  );
}
...
function decodeObjectWithString(payload, pos) {
  return decodeObject(
    payload,
    pos,
    OBJECT_WITH_STRING_HEADER_SIZE,
    OBJECT_WITH_STRING_TYPE_INDEX,
    OBJECT_WITH_STRING_LENGTH_INDEX,
    OBJECT_WITH_STRING_OBIS_ID_INDEX,
    OBJECT_WITH_STRING_GROUP_MASK_INDEX,
    -1,
    -1,
    -1
  );
...
@return: array - [length, obisID as string, groupMask byte, unit byte if available otherwise -1, scaler byte, manntissa int value] or
                 ...
                 depending on the available bytes
****/
function decodeObject(payload, pos, headerSize, typeIndex, lengthIndex, obisIDIndex, groupMaskIndex, unitIndexDependendObisID, scalerIndexDependendObisID, mantissaIndexDependendObisID) {
...
function resolveObisID(groupMask, payload, obisIDIndex, pos) {
  ...
  for (maskBit = OBIS_ID_GROUP_A; maskBit >= OBIS_ID_GROUP_F; maskBit >>= 1) {
    if ((groupMask & maskBit) == maskBit) obisIDLength++;
  }
  ...
  if (payload.length > pos + 1 /*groupMask*/ + obisIDLength + 1 /*scaler*/) {
    ...
    for (maskBit = OBIS_ID_GROUP_A; maskBit >= OBIS_ID_GROUP_F; maskBit >>= 1) {
      ...
      if (maskBit > OBIS_ID_GROUP_F) obisID += OBIS_ID_DELIMITER;
    }
...
function decodeStatusPayload(statusData) {
  ...
  var correctReceivedMeterFilesCounter =
    (correctReceivedMeterFilesCounterBuffer[0] |
      (correctReceivedMeterFilesCounterBuffer[1] << 8) |
      (correctReceivedMeterFilesCounterBuffer[2] << 16) |
      (correctReceivedMeterFilesCounterBuffer[3] << 24)) >>>
    0;
  //convert faulty received meter files with read / CRC errors bytes buffer to number | >>> 0 for UINT32
  var incorrectReceivedMeterFilesCounter =
    (incorrectReceivedMeterFilesCounterBuffer[0] |
      (incorrectReceivedMeterFilesCounterBuffer[1] << 8) |
      (incorrectReceivedMeterFilesCounterBuffer[2] << 16) |
      (incorrectReceivedMeterFilesCounterBuffer[3] << 24)) >>>
    0;
  //convert uploaded meter data messages bytes buffer to number | >>> 0 for UINT32
  var uploadedMeterDataMessagesCounter =
    (uploadedMeterDataMessagesCounterBuffer[0] |
      (uploadedMeterDataMessagesCounterBuffer[1] << 8) |
      (uploadedMeterDataMessagesCounterBuffer[2] << 16) |
      (uploadedMeterDataMessagesCounterBuffer[3] << 24)) >>>
    0;
