...
var payload_parser_version = [2,1,1];
const ERROR_CODE =
{
  0: "DEVICE_READY"
};
  ...
  while(idx < bufferSize)
  {
    ...
    switch(COMMAND_ID[commandId])
    {
      ...
      break;
            ...
            case "COMMAND_ID_GET_AUTO_UPDATE_INSTALLATION":
        data.update_info = data.update_info || {};
        data.update_info.auto_update_installation_en = {};

        data.update_info.auto_update_installation_en.value = {};
        data.update_info.auto_update_installation_en.value = !!(input.bytes[idx++]);
        data.update_info.auto_update_installation_en.unit = "bool";
      break;

      case "COMMAND_ID_GET_PENDING_UPDATE_INFO":
        data.update_info = data.update_info || {};
        data.update_info.update_image = data.update_info.update_image || {};
        data.update_info.update_image.type = {};
        data.update_info.update_image.version = {};

        data.update_info.update_image.type.value = UPDATE_IMAGE_TYPE[input.bytes[idx++]];
        data.update_info.update_image.type.unit = "string";
        data.update_info.update_image.version.value = {};
        data.update_info.update_image.version.value[0] = input.bytes[idx++];
        data.update_info.update_image.version.value[1] = input.bytes[idx++];
        data.update_info.update_image.version.value[2] = input.bytes[idx++];
        data.update_info.update_image.version.unit = "uint";
      break;
