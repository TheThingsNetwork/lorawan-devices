...
var tx_reason = [ "Reserved", "Join Button Pressed", "Cyclic Timer", "Settings", "Joined" ];
var frame_type = [ "Reserved", "Sensor_Data", "Config_Data_Spreading_Factor", "Config_Data_Rejoin", "Config_Data_All", "Device_Info"];
var error_msg = [ "No Error" , "No Sensor Connected", "Sensor Communication Error" ];
var energy_type = [ "Reserved", "Power", "Flow", "Energy Counter", "Volume" ];
  ...
  if (port === 10)
  {
    ...
    switch (decoded.Frame_Type)
    {
      ...
      {              
        ...
        Energy_Data.Value[index_energy_data] = ( Number( "0x" + byteToString( bytes[8] ) + byteToString( bytes[7] ) + byteToString( bytes[6] ) ) / 100.0 );
        ...
        Energy_Data.Value[index_energy_data] = ( Number( "0x" + byteToString( bytes[15] ) + byteToString( bytes[14] ) + byteToString( bytes[13] ) + byteToString( bytes[12] ) + byteToString( bytes[11] ) ) / 10000.0 );   
        ...
        if( bytes.length > ( 0x10 ) )
        {
          ...
          Energy_Data.Value[index_energy_data] = ( Number( "0x" + byteToString( bytes[22] ) + byteToString( bytes[21] ) + byteToString( bytes[20] ) + byteToString( bytes[19] ) + byteToString( bytes[18] ) ) / 10000.0 );
          ...
          Energy_Data.Value[index_energy_data] = ( Number( "0x" + byteToString( bytes[29] ) + byteToString( bytes[28] ) + byteToString( bytes[27] ) + byteToString( bytes[26] ) + byteToString( bytes[25] ) ) / 10000.0 );
        }
      ...
      {         
        ...
        switch ( bytes[3] )
        {
          case 0x00:
          {
            decoded.Spreading_Factor = spreading_factor[(bytes[2])]; 
          } break;
          ...
          case 0x0C:
          {
            decoded.Spreading_Factor = spreading_factor[( (bytes[2]) - 6 )];
          } break;
      ...
      {         
        decoded.Rejoin_Cycle.On_Time = ( bytes[3] << 7 );
      
        decoded.Rejoin_Cycle.Interval = ( ( ( bytes[3] & 0x7F ) << 8  ) | bytes[4] ) ;
      } break;
