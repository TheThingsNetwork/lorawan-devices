...
function CalculatePmAqi(pm2_5, pm10) {
  ...
  function interpolate(v, t) {
    ...
    var i;
    for (i = t.length - 2; i > 0; --i) if (t[i][0] <= v) break;
  ...
  if (result.AQI_2_5 === null) result.AQI = result.AQI_10;
  else if (result.AQI_10 === null) result.AQI = result.AQI_2_5;
  else if (result.AQI_2_5 > result.AQI_10) result.AQI = result.AQI_2_5;
  else result.AQI = result.AQI_10;
...
function Decoder(bytes, port) {
  ...
  if (!(port === 1 || port === 5)) return null;

  var uFormat = bytes[0];
  if (!(uFormat === 0x20 || uFormat === 0x21)) return null;
  ...
  if (flags & 0x10) {
    ...
    if (uFormat === 0x20) decoded.p = (DecodeU16(Parse) * 4) / 100.0;
    ...
    if (tHeat !== null) decoded.tHeatIndex = ((tHeat - 32) * 5) / 9;
  }
