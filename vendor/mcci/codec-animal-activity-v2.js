...
Name:   codec-animal-activity-v2.js
...
function CalculateHeatIndex(t, rh) {
  ...
  if (tHeatEasy + t < 160.0) return ((tHeatEasy - 32) * 5) / 9;
...
function DecodeSflt24(Parse) {
  ...
  if (uExp === 0x7f) {
    if (uMantissa === 0) return bSign ? Number.NEGATIVE_INFINITY : Number.POSITIVE_INFINITY;
    else return Number.NaN;
    // else unless denormal, set the 1.0 bit
  } else if (uExp !== 0) {
    uMantissa += 0x010000;
  } else {
    // denormal: exponent is the minimum
    uExp = 1;
  }
...
function DecodeDownlinkResponse(bytes, port) {
  ...
  if (port === 1) {
    ...
    if (response === 0) decoded.response = 'Success';
    else if (response === 1) decoded.response = 'Invalid Length';
    else if (response === 2) decoded.response = 'Failure';
    else if (response === 3) decoded.response = 'Invalid Range';
  } else if (port === 2) {
    // SW version and HW details.
    ...
    else if (response === 4) decoded.response = 'Sensor Not Connected';
  } else if (port === 3) {
    // fetch the bitmap.
    ...
    if (command === 0x02) {
      ...
      if (responseError === 0) decoded.ResponseError = 'Success';
      else if (responseError === 1) decoded.ResponseError = 'Invalid Length';
      else if (responseError === 2) decoded.ResponseError = 'Failure';
      ...
      if (!(Model === 0)) {
        ...
        else if (Rev === 3) decoded.Rev = 'D';
        else if (Rev === 4) decoded.Rev = 'E';
        else if (Rev === 5) decoded.Rev = 'F';
        else if (Rev === 6) decoded.Rev = 'G';
      } else if (Model === 0) {
        decoded.Model = 4610;
...
function Decoder(bytes, port) {
  ...
  if (!(port === 1) && !(port === 2) && !(port === 3) && !(port === 4)) return null;

  var uFormat = bytes[0];
  if (!(uFormat === 0x26) && !(uFormat === 0x36) && (port === 1 || port === 2 || port === 3)) {
    decoded = DecodeDownlinkResponse(bytes, port);
    return decoded;
  } else if (!(uFormat === 0x26) && !(uFormat === 0x36) && !(port === 1 || port === 2 || port === 3)) return null;
  ...
  if (flags & 0x10) {
    if (uFormat === 0x26) {
      ...
      if (tHeat !== null) decoded.tHeatIndexC = tHeat;
