function decodeUplink(input) {
  ...
  if (v4_1 == '40') {
    ...
    if (conf_p == '9') {
      ack_t = 'Class Setting Ack';
    }
    if (conf_p == '10') {
      ack_t = 'Radio Setting Ack';
    }
    if (conf_p == '11') {
      ack_t = 'Uplink Frequency Ack';
    }
    if (conf_p == '12') {
      ack_t = 'Time Synchronization Ack';
    }
    if (conf_p == '13') {
      ack_t = 'Time Synchronization Request Ack';
    }
    if (conf_p == '14' || conf_p == '15' || conf_p == '16' || conf_p == '17' || conf_p == '18' || conf_p == '19' || conf_p == '20') {
      ack_t = 'Schedulers Setting Ack';
    }
    if (conf_p == '21') {
      ack_t = 'Schedulers Inhibition Ack';
    }
    if (conf_p == '22') {
      ack_t = 'Magnet Control Ack';
    }
    if (conf_p == '10') {
...
function encodeDownlink(input) {
  ...
  var timeZoneFromDB = +4.0; //time zone value from database
  ...
  if (port == 2) {
    if (input.data.status == 20) {
      bytes[0] = 32;
    }
    if (input.data.status == 21) {
      bytes[0] = 33;
    }
    if (input.data.status == 40) {
      bytes[0] = 64;
    }
    if (input.data.status == 41) {
      bytes[0] = 65;
    }
    if (input.data.status == 80) {
      bytes[0] = 128;
    }
    if (input.data.status == 81) {
      bytes[0] = 129;
    }
