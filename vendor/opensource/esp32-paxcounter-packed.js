...
function decodeUplink(input) {
  ...
  if (input.fPort === 3) {
    ...
    data = decode(
      input.bytes,
      [uint8, uint8, int16, uint8, uint8, uint8, uint8, bitmap1, bitmap2, version],
...
var float = function (bytes) {
  ...
  var b = ('00000000' + Number(bytes[0]).toString(2)).slice(-8) + ('00000000' + Number(bytes[1]).toString(2)).slice(-8);
  if (isNegative) {
    var arr = b.split('').map(function (x) {
      return !Number(x);
    });
...
var bitmap1 = function (byte) {
  ...
  return ['adr', 'screensaver', 'screen', 'countermode', 'blescan', 'antenna', 'reserved', 'reserved'].reduce(function (obj, pos, index) {
    obj[pos] = +bm[index];
    return obj;
  }, {});
};
...
var bitmap2 = function (byte) {
  ...
  var i = bytesToInt(byte);
  var bm = ('00000000' + Number(i).toString(2)).substr(-8).split('').map(Number).map(Boolean);
  return ['battery', 'sensor3', 'sensor2', 'sensor1', 'gps', 'bme', 'reserved', 'counter'].reduce(function (obj, pos, index) {
    obj[pos] = +bm[index];
    return obj;
  }, {});
};
